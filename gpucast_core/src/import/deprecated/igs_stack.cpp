...
namespace gpucast {
  ...
  {
    impl_t(std::shared_ptr<nurbssurfaceobject> no)
      : target_(no)
    {}
    ...
    std::map<int, matrix_t>                 matrices_;
...
////////////////////////////////////////////////////////////////////////////////
/* private */ igs_stack::igs_stack(std::shared_ptr<nurbssurfaceobject> target)
  ...
  try {
    if (isdigit(impl_->globalstring_[0])) {
      ...
      if (delim.length() == 1) {
	      ...
	      std::cerr << "igs_stack::evaluateGlobals() : Non supported delimiter" << std::endl;
	      exit(0);
      }
    }
    evalDelimiter();
...
igs_stack::evalInt()
{
  ...
  int res = 0;
  if (len_offset > 0) {
    std::string nr(impl_->globalstring_.begin(), impl_->globalstring_.begin() + len_offset);
    res = atoi(nr.c_str());
...
////////////////////////////////////////////////////////////////////////////////
double
igs_stack::evalDouble()
{
  std::size_t len_offset(0);
  while ((impl_->globalstring_[len_offset] != impl_->globals_.para_delim[0]) &&
...
igs_stack::evalString()
{
  ...
  if (impl_->globalstring_[0] != 'H') {
    ...
    std::string res(impl_->globalstring_.begin(), impl_->globalstring_.begin() + char_count);
...
////////////////////////////////////////////////////////////////////////////////
void igs_stack::insertString(std::string& s)
{
  impl_->string_.push_back(s);
}
...
void igs_stack::addParameterRecord()
{
  ...
  impl_->parameter_.insert(std::make_pair(impl_->para_startindex_, impl_->current_para_));
...
void igs_stack::createDataEntities()
{
  ...
  for (std::vector<igs_data>::const_iterator de = impl_->data_.begin(); de != impl_->data_.end(); ++de, ++index) {
    ...
    case 100 :
      ...
      break;
    case 102 :
      //std::cout << "Creating curve composite entity(102).\n";
    ...
    case 108 :
      ...
      break;
    case 110 :
      //std::cout << "Creating line entity(110).\n";
    ...
    case 116 :
      ...
      break;
    case 120 :
      //std::cout << "Warning: Surface of Revolution not implemented yet" << std::endl;
      break;
    case 124 :
      createMatrix(*de, de_nr); // Transformation Matrix
      break;
    case 126 :
      //std::cout << "Creating curve entity(126).\n";
      createCurve(*de, de_nr); // Rational Curve
      break;
    case 128 :
      //std::cout << "Creating surface entity(128).\n";
      createSurface(*de, de_nr); // Rational Surface
      break;
    case 142 :
      //std::cout << "Creating curve on surface entity(142).\n";
      createCurveOnSurface(*de, de_nr); // Curve on surface entity
      break;
    case 144 :
      //std::cout << "Creating trimmed surface entity(144).\n";
      createTrimmedSurface(*de, de_nr); // Trimmed surface entity
      break;
    case 314 :
      //std::cout << "Creating color entity(314).\n";
    ...
    case 404 :
	  ...
	  break;
    default :
      std::cout << "Warning: Non-implemented entity type" << de->entity_type << std::endl;
    }
  }
  ...
  try {
    // apply trims
    for (std::map<int, igs_trim>::const_iterator i = impl_->trims_.begin(); i != impl_->trims_.end(); ++i) {
      ...
      if (impl_->surfaces_.count(i->second.surface)) {
	      ...
	      if (id > 0) 
        {
	        ...
	        if (outer.size() > 0) 
          {
	          ...
	          if (impl_->curves_.count((int)id) > 0) 
            {
              ...
              loop.push_back (impl_->curves_[(int)id]);
	            ...
	            throw std::runtime_error("Warning: Non-supported type for outer boundary");
	          }
	        }
	      }
	        ...
	        if (impl_->curves_.count((int)inner_id)) 
          {
            ...
            {
		          impl_->surfaces_[i->second.surface].add(impl_->composites_[int(inner_id)]);
	          } else {
	            throw std::runtime_error("Warning: Trim parametric not implemented or not defined");
	          }
	        }
	      }
      } else {
	      throw std::runtime_error("Warning: Surface to be trimmed not implemented or incorrect");      }
       }
    } // try
    ...
    for (std::map<int,nurbssurface>::const_iterator i = impl_->surfaces_.begin(); i != impl_->surfaces_.end(); ++i)
    {
...
void igs_stack::createArc(igs_data const& data, std::size_t /*index*/)
{
  ...
  std::shared_ptr<nurbscurve2d> nc(new nurbscurve2d);
...
void igs_stack::createCurve(igs_data const& data, std::size_t index)
{
  ...
  switch (rational) {
  case 0 : coords = 2; break;
  case 1 : coords = 3; break;
  }
  ...
  for (std::size_t i = idx1; i < idx2; ++i, ++coord) {
    ...
    if (p.string_values.count((int)i)) {
      if (coord%coords != 2) {
	      ...
	      (*ins)[coord%coords] = convertString(p.numeric_values[(int)i], p.string_values[(int)i]);
      }
    } else {
      if (coord%coords != 2) {
...
void igs_stack::createLine(igs_data const& data, std::size_t index)
{
  ...
  for (std::size_t i = 1; i < 7; ++i)
  {
    ...
    if (coord <= 1) // only x/y or u/v coordinate
      ...
      if (p.string_values.count((int)i)) {
        // try to recover string to double
        points[pt_index][coord] = convertString(p.numeric_values[(int)i], p.string_values[(int)i]);
      } else {
  ...
  try {
    ...
    if (impl_->surfaces_.count(trim.surface)) {
      ...
      if (0 != (std::size_t)p.numeric_values[4]) {
	      igs_curve_on_surface const& cos = impl_->curve_surfs_[int(p.numeric_values[4])];

	      // get composite
	      std::size_t composite_index;
	      switch (cos.preferred) {
	      case 1  : composite_index = cos.bmap; break;
	      case 2  : composite_index = cos.cmap; break;
	      case 3  : composite_index = cos.bmap; break;
	      case 0  : composite_index = cos.bmap; break;
	      default : throw std::runtime_error("Warning: Unsupported trimming type");
	      }
      ...
      for (std::size_t i = 5; i < 5 + cos_cnt; ++i) {
	      ...
	      if (impl_->curve_surfs_.count((int)p.numeric_values[(int)i])) {
	        igs_curve_on_surface const& cos = impl_->curve_surfs_[(int)p.numeric_values[(int)i]];
      ...
      throw std::runtime_error("Warning: Surface to be trimmed is not supported yet");
    }
...
////////////////////////////////////////////////////////////////////////////////
double
igs_stack::convertString(double d, std::string& s)
{
...
////////////////////////////////////////////////////////////////////////////////
void
igs_stack::dline_action(std::string const& str)
{
  ...
  switch (impl_->dataline_) {
    ...
    case 0 :
      for (unsigned i = 0; i < 9; ++i) {
	      std::string s(line_offset + begin + i*width, line_offset + begin + (i+1)*width);
	      insertInt(s);
      }
...
////////////////////////////////////////////////////////////////////////////////
void
igs_stack::pline_action(std::string const& str)
{
...
////////////////////////////////////////////////////////////////////////////////
void
igs_stack::float_action(double const& value)
{
  impl_->current_para_.numeric_values.insert(std::make_pair(impl_->current_parapos_, value));
...
////////////////////////////////////////////////////////////////////////////////
void
igs_stack::string_action(std::string const& str)
{
...
////////////////////////////////////////////////////////////////////////////////
void
igs_stack::pstring_action(std::string const& str)
{
  // set back because of string length value
  --impl_->current_parapos_;

  // add string for current index
  impl_->current_para_.string_values.insert(std::make_pair(impl_->current_parapos_, str));
...
////////////////////////////////////////////////////////////////////////////////
void
igs_stack::int_action(int value)
{
  impl_->int_.push_back(value);
}
