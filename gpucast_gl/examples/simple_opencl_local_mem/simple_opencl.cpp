...
// opencl header
#include <glpp/opencl/clplatform.hpp>
#include <glpp/opencl/cldevice.hpp>
#include <glpp/opencl/clcontext.hpp>
...
public :
  ...
  void initcl()
  {
    ...
    // Make kernel
    cl::Kernel kernel(program, "run_kernel", &err);
    if ( err != CL_SUCCESS ) 
    {
    ...
    // curve data
    std::vector<glpp::vec4f> points(BLOCKSIZE);
    points[0] = glpp::vec4f(1.0, 0.0, 0.0, 1.0);
    points[1] = glpp::vec4f(3.0, 4.0,-3.0, 1.0);
    points[2] = glpp::vec4f(2.0, 2.0, 7.0, 1.0);
    points[3] = glpp::vec4f(0.0, 4.0, 0.0, 1.0);

 
    // Create memory buffers
    cl::Buffer bufferA (context, CL_MEM_READ_ONLY, BLOCKSIZE * 4 * sizeof(float), 0, &err);
    if ( err != CL_SUCCESS ) {
    ...
    cl::Buffer bufferB (context, CL_MEM_WRITE_ONLY, _samples * 4 * sizeof(float), 0, &err);
    if ( err != CL_SUCCESS ) {
      std::cerr << "Error allocating CL memory : " << err << std::endl;
    } else {
      std::cout << "Allocating memory ok." << std::endl;
    ...
    queue.enqueueWriteBuffer (bufferA, CL_TRUE, 0, BLOCKSIZE * 4 * sizeof(float), &points[0]);
    queue.enqueueWriteBuffer (bufferB, CL_TRUE, 0, _samples * 4 * sizeof(float), &samples[0]);
