...
#include <glpp/test/dynamic_test.hpp>
#include <glpp/test/dynamic_transform.hpp>
...
public :
      ...
      _camera   ()
  {
    ...
    _test.add ( glpp::dynamic_transform_ptr(new glpp::dynamic_translation ( 2000, glpp::vec3f(0.0f, 0.0f, 0.0f), glpp::vec3f(2.0f, 2.0f, 2.0f))) );
    _test.add ( glpp::dynamic_transform_ptr(new glpp::dynamic_translation ( 2000, glpp::vec3f(2.0f, 2.0f, 2.0f), glpp::vec3f(0.0f, 0.0f, 0.0f))) );
    _test.add ( glpp::dynamic_transform_ptr(new glpp::dynamic_rotation    ( 2000, 0 )));
    _test.add ( glpp::dynamic_transform_ptr(new glpp::dynamic_rotation    ( 2000, 1 )));
    _test.add ( glpp::dynamic_transform_ptr(new glpp::dynamic_rotation    ( 2000, 2 )));
    _test.add ( glpp::dynamic_transform_ptr(new glpp::dynamic_scaling     ( 2000, glpp::vec3f(1.0f, 1.0f, 1.0f), glpp::vec3f(6.0f, 3.0f, 2.0f))) );
    _test.add ( glpp::dynamic_transform_ptr(new glpp::dynamic_scaling     ( 2000, glpp::vec3f(6.0f, 3.0f, 2.0f), glpp::vec3f(1.0f, 1.0f, 1.0f))) );
  }
  ...
  void draw()
  {
    ...
    glpp::matrix4f model = _test.get_current_modeltransform ();
    _test.step();

    glpp::matrix4f viewtransform  =  glpp::make_translation(_trackball->shiftx(), _trackball->shifty(), _trackball->distance()) *
                                     _trackball->rotation();

    glpp::matrix4f proj = glpp::perspective(60.0f, 1.0f, 1.0f, 1000.0f);
    glpp::matrix4f mv   = view * viewtransform * model;
    ...
    if ( _test.finished() )
    {
      glpp::glutwindow::instance().leave(boost::bind(&application::leave, this));
    }
  }
  
  void leave()
  {
    std::cout << "total time  : " << _test.get_total_time() << std::endl;
    std::cout << "frames      : " << _test.frames() << std::endl;
    std::cout << "fps         : " << _test.get_average_fps() << std::endl;
    system("pause");
  }
...
public :
  ...
  glpp::dynamic_test                  _test;
};


int main(int argc, char** argv)
{
  glpp::glutwindow::init(argc, argv, 1024, 1024, 0, 0, 4, 2, true);
  glpp::glutwindow::instance().printinfo(std::cout);

  glewExperimental = true;
  glewInit();

  if (!glpp::set_vsync(false))
  {
    std::cerr << "Failed to disable vsync.\n" << std::endl;
    exit(0);
  }
