...
void get_nearfar(float& nearplane, float& farplane)
{
  float frustum[6][4];
  float   proj[16];
  float   modl[16];
  ...
  // compute distance of near and far plane
  farplane =   frustum[4][3] - modl[14];
  nearplane = -frustum[5][3] - modl[14];
}
