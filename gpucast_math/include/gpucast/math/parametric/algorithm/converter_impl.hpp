...
inline void
converter<point_t>::convert( gpucast::math::nurbscurve<point_t> const& nc,
...
template <typename insert_iterator_t>
inline void
converter<point_t>::convert( gpucast::math::nurbssurface<point_t> const& nurbs,
                            insert_iterator_t                 target ) const
{
  // copy control points
  ...
  std::multiset<value_type> orig_knotU(nurbs.knotvector_u().begin(), nurbs.knotvector_u().end());
  std::multiset<value_type> orig_knotV(nurbs.knotvector_v().begin(), nurbs.knotvector_v().end());

  std::size_t orderU = nurbs.order_u();
  std::size_t orderV = nurbs.order_v();

  // orignal polygon
  pointmesh2d<point_t> orig(nurbs.points().begin(), nurbs.points().end(),	orig_cols, orig_rows);
  ...
  for (std::size_t r = 0; r < parts_v; ++r)
  {
    for (std::size_t c = 0; c < parts_u; ++c)
    {
      ...
      pointmesh2d<point_t> subpatch = knotins_complete.subpatch(index_u, index_u + orderU - 1, index_v, index_v + orderV - 1);
      ...
      /*patch.set_patch_parameter(*para_min_u, *para_max_u, *para_min_v, *para_max_v);
      patch.set_nurbs_parameter(*new_knotU.begin(), *new_knotU.rbegin(), *new_knotV.begin(), *new_knotV.rbegin());

      // apply trimcurves here!
      for (std::vector<beziercurve<double3_t> >::iterator i = impl_->curves_.begin(); i < impl_->curves_.end(); ++i)
      {
	      patch.add(*i);
      }
...
template <typename insert_iterator_t>
inline void
converter<point_t>::convert ( nurbsvolume_type const& volume, insert_iterator_t ins_iter, std::vector<beziervolumeindex>& indices) const
{
  typedef typename std::set<value_type>::const_iterator set_const_iterator;

  std::set<value_type> unique_knots_u (volume.knotvector_u().begin(), volume.knotvector_u().end());
  std::set<value_type> unique_knots_v (volume.knotvector_v().begin(), volume.knotvector_v().end());
  std::set<value_type> unique_knots_w (volume.knotvector_w().begin(), volume.knotvector_w().end());

  std::vector<value_type> unique_knots_u_as_vector ( unique_knots_u.begin(), unique_knots_u.end());
  std::vector<value_type> unique_knots_v_as_vector ( unique_knots_v.begin(), unique_knots_v.end());
  std::vector<value_type> unique_knots_w_as_vector ( unique_knots_w.begin(), unique_knots_w.end());
  ...
  pointmesh3d<point_t> mesh_u (targetsize_u, volume.numberofpoints_v(), volume.numberofpoints_w());
  for (unsigned int v = 0; v != volume.numberofpoints_v(); ++v)
  {
    for (unsigned int w = 0; w != volume.numberofpoints_w(); ++w)
    {
      ...
      std::multiset<value_type> knots (volume.knotvector_u().begin(), volume.knotvector_u().end());
  ...
  pointmesh3d<point_t> mesh_uv (targetsize_u, targetsize_v, volume.numberofpoints_w());
  for (unsigned int u = 0; u != targetsize_u; ++u)
  {
    for (unsigned int w = 0; w != volume.numberofpoints_w(); ++w)
    {
      std::vector<point_t>      curve = mesh_u.submesh(1, u, w);
      std::multiset<value_type> knots (volume.knotvector_v().begin(), volume.knotvector_v().end());
  ...
  for (unsigned int u = 0; u != targetsize_u; ++u)
  {
    for (unsigned int v = 0; v != targetsize_v; ++v)
    {
      ...
      std::multiset<value_type> knots(volume.knotvector_w().begin(), volume.knotvector_w().end());
      knot_insertion (curve, knots, volume.order_w());
      mesh_uvw.submesh(curve.begin(), 2, u, v);
    }
  }

  typename beziervolume_type::parameter_type uvwglobalmin ( unique_knots_u_as_vector[0], unique_knots_v_as_vector[0], unique_knots_w_as_vector[0] );
  typename beziervolume_type::parameter_type uvwglobalmax ( unique_knots_u_as_vector[knotspans_u], unique_knots_v_as_vector[knotspans_v], unique_knots_w_as_vector[knotspans_w] );
  ...
  for (int u = 0; u != int(knotspans_u); ++u)
  {
    for (int v = 0; v != int(knotspans_v); ++v)
    {
      ...
      {
        typename beziervolume_type::parameter_type uvwmin ( unique_knots_u_as_vector[u  ], unique_knots_v_as_vector[v  ], unique_knots_w_as_vector[w  ] );
        typename beziervolume_type::parameter_type uvwmax ( unique_knots_u_as_vector[u+1], unique_knots_v_as_vector[v+1], unique_knots_w_as_vector[w+1] );

        // create control point mesh of target size
        pointmesh3d<point_t>  bmesh ( volume.order_u(), volume.order_v(), volume.order_w());
...
inline void
converter<point_t>::knot_insertion(point_container_t& P, std::multiset<typename point_t::value_type>& knots, std::size_t order) const
{
...
template <typename point_container_t>
inline void
converter<point_t>::knot_insertion(point_container_t& P, std::multiset<value_type>& knots, std::size_t order, value_type t) const
{
  typedef typename point_t::value_type value_type;
  ...
  for (typename std::vector<point_t>::iterator i = Qw.begin(); i != Qw.end(); ++i)
  {
