...
*
********************************************************************************/
#ifndef GPUCAST_MATH_DECASTELJAU_HPP
#define GPUCAST_MATH_DECASTELJAU_HPP
...
#include <gpucast/math/parametric/algorithm/space_adapter.hpp>
...
class decasteljau : public evaluator<point_t>
{
public : //typedefs
...
public : // method
                                ...
                                point_t& point) const
  {
    ...
    std::transform(points, points + order, tmp.begin(), point_to_homogenous_space_adapter<point_t>());
  ...
  /* virtual */ void operator()(/* pointer to first point in rowwise control point grid */
                                ...
                                /* w-parameter for point to evaluate */
		                            ...
		                            point_t& point) const
  {
      pointmesh3d<point_t> mesh(points, points + order_u * order_v * order_w, order_u, order_v, order_w);

      // transform control points to homogenous space
      std::for_each(mesh.begin(), mesh.end(), std::mem_fn(&point_t::project_to_homogenous));
      ...
      for (std::size_t jv = 0; jv != order_v; ++jv)
      {
        for (std::size_t jw = 0; jw != order_w; ++jw)
        {
          for (std::size_t i = 0; i != order_u-1; ++i)
          {
            ...
            {
              mesh(j, jv, jw) = (value_type(1) - u) * mesh(j, jv, jw) + u * mesh(j+1, jv, jw);
            }
          }
        }
      }
      ...
      for (std::size_t ju = 0; ju != order_u; ++ju)
      {
        for (std::size_t jw = 0; jw != order_w; ++jw)
        {
          for (std::size_t i = 0; i != order_v-1; ++i)
          {
            ...
            {
              mesh(ju, j, jw) = (value_type(1) - v) * mesh(ju, j, jw) + v * mesh(ju, j+1, jw);
            }
          }
        }
      }
      ...
      for (std::size_t ju = 0; ju != order_u; ++ju)
      {
        for (std::size_t jv = 0; jv != order_v; ++jv)
        {
          for (std::size_t i = 0; i != order_w-1; ++i)
          {
            ...
            {
              mesh(ju, jv, j) = (value_type(1) - w) * mesh(ju, jv, j) + w * mesh(ju, jv, j+1);
            }
          }
        }
      }
