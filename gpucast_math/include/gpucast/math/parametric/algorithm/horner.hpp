...
*
********************************************************************************/
#ifndef GPUCAST_MATH_HORNER_HPP
#define GPUCAST_MATH_HORNER_HPP
...
class horner : public evaluator<point_t>
{
public : //typedefs
  ...
  typedef typename evaluator<point_t>::const_point_iterator const_point_iterator;
...
public : // method
                                ...
                                point_t& dt) const
  {
    ...
    dt = value_type(order-1) * (((tmp0.weight() * tmp1.weight()) / (point.weight() * point.weight()))) * (tmp1.as_euclidian() - tmp0.as_euclidian());
  ...
  /////////////////////////////////////////////////////////////////////////////
  /* virtual */ point_t operator()(/* pointer to first point in rowwise control point grid */
                                  const_point_iterator points,
                                  /* order in u dir */
			                            std::size_t order_u,
			                            /* order in v dir */
			                            std::size_t order_v,
			                            /* u-parameter for point to evaluate */
			                            value_type u,
			                            /* v-parameter for point to evaluate */
			                            value_type v) const
	{
    point_t tmp;
  ...
  /* virtual */ void operator()(/* pointer to first point in rowwise control point grid */
                                const_point_iterator points,
                                /* order in u dir */
			                          std::size_t order_u,
			                          /* order in v dir */
    ...
    point_t u1_0 = (*(points + order_u    )).as_homogenous() * (value_type(1.0) - u);
    point_t u1_1 = (*(points + order_u + 1)).as_homogenous() * (value_type(1.0) - u);
    ...
    for (std::size_t i = 1; i <= deg_u - 2; ++i)
    {
      ...
      u0_0 = (u0_0 + un * bcu * (*(points + i    )).as_homogenous())           * (value_type(1.0) - u);
      u0_1 = (u0_1 + un * bcu * (*(points + i + 1)).as_homogenous())           * (value_type(1.0) - u);

      u1_0 = (u1_0 + un * bcu * (*(points + order_u + i    )).as_homogenous()) * (value_type(1.0) - u);
      u1_1 = (u1_1 + un * bcu * (*(points + order_u + i + 1)).as_homogenous()) * (value_type(1.0) - u);
    }
    ...
    for (std::size_t i = 1; i <= deg_v - 2; ++i)
    {
      bcu = 1.0;
      un = 1.0;
      ui_0 = (*(points + (i+1) * order_u)).as_homogenous()     * (value_type(1.0) - u);
      ui_1 = (*(points + (i+1) * order_u + 1)).as_homogenous() * (value_type(1.0) - u);

      for (std::size_t j = 1; j <= deg_u-2; ++j)
      {
        un = un * u;
        bcu = bcu * (value_type(deg_u - j) / value_type(j));
        ui_0 = (ui_0 + un * bcu * (*(points + (i+1) * order_u + j    )).as_homogenous()) * (value_type(1.0) - u);
        ui_1 = (ui_1 + un * bcu * (*(points + (i+1) * order_u + j + 1)).as_homogenous()) * (value_type(1.0) - u);
      }
      ui_0 = ui_0 + un * u * (*(points + (i+1) * order_u + deg_u - 1)).as_homogenous();
      ui_1 = ui_1 + un * u * (*(points + (i+1) * order_u + deg_u    )).as_homogenous();
    ...
    bcu = 1.0;
    un = 1.0;
    ui_0 = (*(points + deg_v * order_u    )).as_homogenous() * (value_type(1.0) - u);
    ui_1 = (*(points + deg_v * order_u + 1)).as_homogenous() * (value_type(1.0) - u);

    for (std::size_t i = 1; i <= deg_u-2; ++i)
    {
      un = un * u;
      bcu = bcu * (value_type(deg_u-i) / value_type(i));
      ui_0 = (ui_0 + un * bcu * (*(points + deg_v * order_u + i    )).as_homogenous()) * (value_type(1.0) - u);
      ui_1 = (ui_1 + un * bcu * (*(points + deg_v * order_u + i + 1)).as_homogenous()) * (value_type(1.0) - u);
    }

    ui_0 = ui_0 + un * u * (*(points + deg_v * order_u + deg_u - 1)).as_homogenous();
    ui_1 = ui_1 + un * u * (*(points + deg_v * order_u + deg_u    )).as_homogenous();
    ui = (value_type(1.0) - u) * ui_0 + u * ui_1;
  ...
  /* virtual */ void operator()(/* pointer to first point in rowwise control point grid */
                                ...
                                /* order in u dir */
			                          ...
			                          /* resulting point at [u,v] */
			                          point_t& point,
			                          /* resulting partial derivative according to u */
			                          point_t& du,
			                          /* resulting partial derivative according to v */
			                          point_t& dv) const
	{
    value_type bcu = 1.0;
    value_type un = 1.0;
    ...
    point_t v0_1 = (*(points + order_u    )).as_homogenous() * (value_type(1.0) - v);
    point_t v1_0 = (*(points + 1          )).as_homogenous() * (value_type(1.0) - v);
    point_t v1_1 = (*(points + order_u + 1)).as_homogenous() * (value_type(1.0) - v);
    ...
    for (std::size_t i = 1; i <= deg_v - 2; ++i)
    {
      ...
      v0_0 = (v0_0 + vn * bcv * (*(points + (i  ) * order_u    )).as_homogenous()) * (value_type(1.0) - v);
      v0_1 = (v0_1 + vn * bcv * (*(points + (i+1) * order_u    )).as_homogenous()) * (value_type(1.0) - v);

      v1_0 = (v1_0 + vn * bcv * (*(points + (i  ) * order_u + 1)).as_homogenous()) * (value_type(1.0) - v);
      v1_1 = (v1_1 + vn * bcv * (*(points + (i+1) * order_u + 1)).as_homogenous()) * (value_type(1.0) - v);
    }

    v0_0 = v0_0 + vn * v * (*(points + (deg_v-1) * order_u    )).as_homogenous();
    v0_1 = v0_1 + vn * v * (*(points + (deg_v  ) * order_u    )).as_homogenous();
    v1_0 = v1_0 + vn * v * (*(points + (deg_v-1) * order_u + 1)).as_homogenous();
    ...
    for (std::size_t i = 1; i <= deg_u - 2; ++i)
    {
      ...
      vi_1 = (*(points + order_u + i + 1)).as_homogenous() * (value_type(1.0) - v);

      for (std::size_t j = 1; j <= deg_v-2; ++j)
      {
        vn = vn * v;
        bcv = bcv * (value_type(deg_v - j) / value_type(j));
        vi_0 = (vi_0 + vn * bcv * (*(points + (j  ) * order_u + i + 1)).as_homogenous()) * (value_type(1.0) - v);
        vi_1 = (vi_1 + vn * bcv * (*(points + (j+1) * order_u + i + 1)).as_homogenous()) * (value_type(1.0) - v);
      }
      vi_0 = vi_0 + vn * v * (*(points + (deg_v-1) * order_u + i + 1)).as_homogenous();
      vi_1 = vi_1 + vn * v * (*(points + (deg_v  ) * order_u + i + 1)).as_homogenous();
    ...
    vi_1 = (*(points + order_u + deg_u      )).as_homogenous() * (value_type(1.0) - v);

    for (std::size_t i = 1; i <= deg_v-2; ++i)
    {
      vn = vn * v;
      bcv = bcv * (value_type(deg_v-i) / value_type(i));
      vi_0 = (vi_0 + vn * bcv * (*(points + (i  ) * order_u + deg_u)).as_homogenous()) * (value_type(1.0) - v);
      vi_1 = (vi_1 + vn * bcv * (*(points + (i+1) * order_u + deg_u)).as_homogenous()) * (value_type(1.0) - v);
     }

    vi_0 = vi_0 + vn * v * (*(points + (deg_v-1) * order_u + deg_u)).as_homogenous();
    vi_1 = vi_1 + vn * v * (*(points + (deg_v  ) * order_u + deg_u)).as_homogenous();
    vi = (value_type(1.0) - v) * vi_0 + v * vi_1;
  ...
  /* virtual */ void operator()(/* pointer to first point in rowwise control point grid */
                          ...
                          /* order in w dir */
	                        ...
	                        /* v-parameter for point to evaluate */
	                        value_type v,
                          /* w-parameter for point to evaluate */
	                        value_type w,
	                        /* resulting point at [u,v,w] */
	                        point_t& point) const
  {
    // helper like binomial coefficients and t^n
      ...
      for (std::size_t j = 0; j != order_v; ++j)
      {
        ...
        for (std::size_t i = 0; i != order_u; ++i)
        {
          if (i == 0) { // first interpolation (1-u)^n
              pu = (points + j * order_u + k * order_u * order_v )->as_homogenous() * one_minus_u;
          } else {
            if (i == order_u - 1) { // last interpolation u^n
              pu += un * u * (points + i + j * order_u + k * order_u * order_v )->as_homogenous();
            } else {  // else follow regular horner scheme
              un  *= u;
              bcu *= value_type(order_u - i) / value_type(i);
              pu = (pu + un * bcu * (points + i + j * order_u + k * order_u * order_v )->as_homogenous()) * one_minus_u;
            }
          }
        }
  ...
  /* virtual */ void operator()(/* pointer to first point in rowwise control point grid */
                          ...
                          /* order in w dir */
		                      ...
		                      /* u-parameter for point to evaluate */
		                      value_type u,
		                      /* value_typeparameter for point to evaluate */
		                      value_type v,
                          /* w-parameter for point to evaluate */
		                      value_type w,
                          /* resulting point at [u,v,w] */
		                      point_t& point,
                          /* first partial derivative in u at [u,v,w] */
		                      point_t& du,
                          /* first partial derivative in v at [u,v,w] */
		                      point_t& dv,
                          /* first partial derivative in w at [u,v,w] */
		                      point_t& dw
                         ) const
  {
    // helper like binomial coefficients and t^n
    ...
    for (std::size_t k = 0; k != order_w; ++k)
    {
      ...
      for (std::size_t j = 0; j != order_v; ++j)
      {
        ...
        pu0 = (points +     j * order_u + k * order_u * order_v )->as_homogenous() * one_minus_u;
        pu1 = (points + 1 + j * order_u + k * order_u * order_v )->as_homogenous() * one_minus_u;
        ...
        for (std::size_t i = 1; i != order_u-2; ++i)
        {
          ...
          pu0 = (pu0 + un * bcu * (points +     i + j * order_u + k * order_u * order_v )->as_homogenous()) * one_minus_u;
          pu1 = (pu1 + un * bcu * (points + 1 + i + j * order_u + k * order_u * order_v )->as_homogenous()) * one_minus_u;
        }
    ...
    // evaluating v,w plane for v resulting in last linear interpolation in w -> to compute first partial derivative in w
