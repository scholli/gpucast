...
*
********************************************************************************/
#ifndef GPUCAST_MATH_BEZIERCURVE_IMPL_HPP
#define GPUCAST_MATH_BEZIERCURVE_IMPL_HPP
...
namespace gpucast { namespace math {
  ...
  template <typename point_t>
  inline void
  beziercurve<point_t>::swap(beziercurve<point_t>& swp)
  {
  ...
  //////////////////////////////////////////////////////////////////////////////
  template <typename point_t>
  inline beziercurve<point_t>&
  beziercurve<point_t>::operator=(beziercurve<point_t> const& cpy)
  {
    beziercurve tmp(cpy);
    swap(tmp);
    return *this;
  ...
  template <typename point_t>
  inline void
  beziercurve<point_t>::add(point_t const& point)
  {
  ...
  void
  beziercurve<point_t>::add (external_point_type const& point)
  {
    add(point_t(point));
  }
  ...
  template <typename point_t>
  inline point_t
  beziercurve<point_t>::evaluate(value_type const& t, evaluator<point_t> const& eval) const
  {
    point_t p;
  ...
  template <typename point_t>
  inline void
  beziercurve<point_t>::evaluate(value_type const& t, point_t& point, evaluator<point_t> const& eval) const
  {
  ...
  template <typename point_t>
  inline void
  beziercurve<point_t>::evaluate(value_type const& t, point_t& point, point_t& dt, evaluator<point_t> const& eval) const
  {
  ...
  template <typename point_t>
  inline std::size_t
  beziercurve<point_t>::degree() const
  {
    return _deg;
  }
  ...
  template <typename point_t>
  inline std::size_t
  beziercurve<point_t>::order() const
  {
    return _deg + 1;
  }
    ...
    if ( _deg < 2 )
    {
      ...
      for (std::size_t i = 0; i != _deg; ++i)
      {
        for (std::size_t j = i+1; j != _deg; ++j)
        {
          ...
          d0.normalize();
          d1.normalize();
          alpha = std::max(alpha, value_type(1) - std::max(value_type(0), dot(d0, d1)) );
        }
      }
      return alpha;
    }
  ...
  template <typename point_t>
  inline beziercurve<point_t>
  beziercurve<point_t>::scaled_hodograph() const
  {
    ...
    for (std::size_t k = 0; k <= 2 * _deg - 2; ++k) {
      ...
      for (std::size_t i = s_idx; i <= e_idx ;++i) {
	      value_type alpha = value_type(k-2*i+1) * value_type(binomial_coefficient(_deg, i)) * value_type(binomial_coefficient(_deg, k-i+1));
    ...
    // set weights to 1
    for (point_iterator i = hodograph._points.begin(); i < hodograph._points.end(); ++i) {
      (*i).weight(1);
    }
  ...
  template <typename point_t>
  inline beziercurve<point_t>
  beziercurve<point_t>::hodograph() const
  {
  ...
  beziercurve<point_t>::nishita(std::size_t coord) const
  {
    ...
    {
      point<value_type,2> equidistant_control_point (value_type(i)/value_type(_deg), /* equidistant on first coordinate*/
                                                    _points[i][coord],              /* value on y axis */
                                                    _points[i].weight());           /* keep weight, although unspecified for rational curves*/
  ...
  beziercurve<point_t>::as_polynomial ( std::size_t coord ) const
  {
    ...
    for ( std::size_t i = 0; i != order(); ++i) 
    {
      ...
      fx += _points[i][coord] * value_type( gpucast::math::binomial_coefficient(n, i)) *  gpucast::math::pow(t, i) *  gpucast::math::pow(one_minus_t, n-i);
    }
  ...
  template <typename point_t>
  inline void
  beziercurve<point_t>::split(value_type t, beziercurve& left, beziercurve& right) const
  {
    left = *this;
    right = *this;
  ...
  inline void
  beziercurve<point_t>::split(parameter_container_t const& ordered_params, curve_container_t& curvesegments) const
  {
      ...
      default : // multiple splits
        {
          for (std::size_t i = 0; i != params.size(); ++i)
          {
            ...
            if ( i == params.size() - 1 )
            {
              ...
              tmp.front() = connection;                 // set first point of segment to last point of preceding segment
            ...
            {
              beziercurve<point_t> tmp(*this);
              tmp.clip_lr(params.at(i), params.at(i+1));// clip left and right part
              tmp.front() = connection;                 // set first point of segment to last point of preceding segment
              connection  = tmp.back();                 // save last point for continuity
  ...
  template <typename point_t>
  inline void
  beziercurve<point_t>::extrema(std::size_t dim, std::set<value_type>& roots, std::size_t iterations) const
  {
    ...
    {
      beziercurve2_type derivative_i = nth_derivative_curves.back().hodograph().nishita(point_t::v);
    ...
    for (int i = int(nth_derivative_curves.size()) - 1; i >= 0; --i)
    {
      ...
      for (std::size_t v = 0; v < intervals.size(); ++v)
      {
        ...
        nth_derivative_curves[i].bisect(point_t::v, 0.0, isroot, root, intervals[v], iterations);
  ...
  template <typename point_t>
  bool
  beziercurve<point_t>::weak_monotonic ( std::size_t dim ) const
  {
    if (_points.size() < 3)
    {
      return true;
  ...
  template <typename point_t>
  inline bool
  beziercurve<point_t>::assert_extremum ( std::size_t               dim,
                                          value_type const&         t,
  ...
  template <typename point_t>
  inline void
  beziercurve<point_t>::bisect(std::size_t        dim,
  ...
  template <typename point_t>
  inline void
  beziercurve<point_t>::optbisect(point_t const& point,
                                  std::size_t fx,
                                  std::size_t x,
                                  bool& intersects,
                                  std::size_t& iters,
                                  ...
                                  std::size_t max_iters ) const
  {
    ...
    if (!(interval<value_type>(pmin[fx], pmax[fx],  gpucast::math::included,  gpucast::math::included).in(point[fx]))) {
      iters = 0;
      intersects = false;
      return;
    }
  ...
  beziercurve<point_t>::minimum(std::size_t dim) const
  {
    minimal_coordinates<point_t> res = std::for_each(_points.begin(), _points.end(), minimal_coordinates<point_t>(dim));
    return res.result();
  }
  ...
  template <typename point_t>
  inline typename point_t::value_type
  beziercurve<point_t>::maximum(std::size_t dim) const
  {
    maximal_coordinates<point_t> res = for_each(_points.begin(), _points.end(), maximal_coordinates<point_t>(dim));
    return res.result();
  }
  ...
  template <typename point_t>
  inline bool
  beziercurve<point_t>::is_constant(std::size_t dim, value_type tolerance) const
  {
    if (!_points.empty())
    {
  ...
  template <typename point_t>
  inline bool
  beziercurve<point_t>::is_rational() const
  {
    for (std::size_t i = 0; i < _points.size(); ++i)
    {
      if ( _points[i].weight() != value_type(1) ) return true;
    }
    return false;
  }
  ...
  template<typename point_t>
  inline bool
  beziercurve<point_t>::is_increasing(std::size_t dim) const
  {
  ...
  beziercurve<point_t>::elevate()
  {
    ...
    if (_points.size() > 1) {
      ...
      for (std::size_t i = 1; i < _points.size(); ++i) {
	      ...
	      point_t p_in_homogen = alpha * _points[i-1].as_homogenous() + value_type(1.0 - alpha) * _points[i].as_homogenous();
  ...
  template <typename point_t>
  inline void
  beziercurve<point_t>::translate(point_t const& t)
  {
    for (point_iterator i = _points.begin(); i!= _points.end(); ++i)
    {
  ...
  template <typename point_t>
  inline void
  beziercurve<point_t>::bbox_simple(axis_aligned_boundingbox<point_t>& bbox) const
  {
    assert(order() > 0);
  ...
  template <typename point_t>
  inline void
  beziercurve<point_t>::bbox_tight(axis_aligned_boundingbox<point_t>& bbox) const
  {
    ...
    for (typename std::set<value_type>::const_iterator i = expnts.begin(); i != expnts.end(); ++i)
    {
      point_t extrema;
  ...
  //////////////////////////////////////////////////////////////////////////////
  template <typename point_t>
  inline typename std::vector<point_t>::iterator
  beziercurve<point_t>::begin() {
  ...
  beziercurve<point_t>::front() const {
  ...
  template <typename point_t>
  inline point_t const&
  beziercurve<point_t>::back() const {
    return _points.back();
  }
  ...
  template <typename point_t>
  inline point_t&
  beziercurve<point_t>::operator[](std::size_t i)
  {
    return _points[i];
  }
  ...
  template <typename point_t>
  inline point_t const&
  beziercurve<point_t>::operator[](std::size_t i) const
  {
    return _points[i];
  }
  ...
  template <typename point_t>
  inline void
  beziercurve<point_t>::intersect_chull(unsigned      def,
                                        unsigned      val,
                                        value_type&   t1,
                                        value_type&   t2)
  {
      ...
      for (const_point_iterator j = i + 1; j != _points.end(); ++j)
      {
        ...
        if ((fst[val] < -std::numeric_limits<value_type>::epsilon() &&
             ...
             snd[val] < -std::numeric_limits<value_type>::epsilon()))
          {
            ...
            value_type xi = ((dx * fst[def] - fst[val]) * (snd[def] - fst[def])) / (snd[val] - fst[val]);
        ...
        if(std::abs(fst[val]) <= std::numeric_limits<value_type>::epsilon())
          {
            if ((*i)[def] < mn) mn = (*i)[def];
            if ((*i)[def] > mx) mx = (*i)[def];
          }
      }
    }
    t1 = mn;
  ...
  template <typename point_t>
  inline std::ostream&
  operator<<(std::ostream& os, beziercurve<point_t> const& rhs)
  {
    rhs.print(os);
    return os;
  }
...
#endif // GPUCAST_MATH_BEZIERCURVE_IMPL_HPP
