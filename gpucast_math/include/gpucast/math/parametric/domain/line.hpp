...
*
********************************************************************************/
#ifndef GPUCAST_MATH_LINE_HPP
#define GPUCAST_MATH_LINE_HPP
...
public :
  ...
  line ( point_t const& a, point_t const& b )
    : start_ ( a ),
      end_   ( b )
  {}
  ...
  bool intersect ( line const& b, point_t& intersect ) const
  {
    ...
    value_t s = ((ax - bx) * d_ay + (by - ay) * d_ax ) / (d_bx*d_ay - d_by*d_ax);
    value_t t = ((ax - bx) * d_by + (by - ay) * d_bx ) / (d_ay*d_bx - d_ax*d_by);
  ...
  void print(std::ostream& os ) const
  {
    ...
    {
      os << "line from " << start_.x << ", " << start_.y << " to " << end_.x << ", " << end_.y << std::endl;
    } else {
      os << "line from " << end_.x << ", " << end_.y << " to " << start_.x << ", " << start_.y << std::endl;
    }
  }
