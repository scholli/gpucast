...
*
********************************************************************************/
#ifndef GPUCAST_MATH_MATRIX_IMPL_HPP
#define GPUCAST_MATH_MATRIX_IMPL_HPP
...
template <typename value_t, unsigned NROWS, unsigned NCOLS>
void
matrix<value_t, NROWS, NCOLS>::operator/= ( value_type const& s )
{
  for ( std::size_t r = 0; r != NROWS; ++r ) {
    for ( std::size_t c = 0; c != NCOLS; ++c ) {
      _data[r][c] /= s;
...
template <typename value_t, unsigned NROWS, unsigned NCOLS>
void
matrix<value_t, NROWS, NCOLS>::validate ( value_t const& eps )
{
  for ( std::size_t r = 0; r != NROWS; ++r ) {
    for ( std::size_t c = 0; c != NCOLS; ++c ) {
...
template <typename value_t, unsigned NROWS, unsigned NCOLS>
std::set<value_t>
matrix<value_t, NROWS, NCOLS>::eigenvalues ( value_t const& min, value_t const& max, value_t const& tolerance ) const
{
  if (NCOLS == NROWS) {
...
matrix<value_t, NROWS, NCOLS>::eigenvectors ( value_t const& min, value_t const& max, value_t const& eps ) const
{
  if (NCOLS == NROWS) {
...
template <typename value_t, unsigned NROWS, unsigned NCOLS>
/* virtual */ void
matrix<value_t, NROWS, NCOLS>::print         ( std::ostream& os ) const
{
  for ( std::size_t r = 0; r != NROWS; ++r )
  {
    os << "[ ";
...
template <typename value_t, unsigned NROWS, unsigned NCOLS>
/* virtual */ void      
matrix<value_t, NROWS, NCOLS>::write ( std::ostream& os ) const
{
...
template <typename value_t, unsigned NROWS, unsigned NCOLS>
/* virtual */ void      
matrix<value_t, NROWS, NCOLS>::read ( std::istream& is )
{
...
//////////////////////////////////////////////////////////////////////////////
template <typename value_t, unsigned NROWS, unsigned NCOLS>
matrix<value_t, NROWS, NCOLS>
matrix<value_t, NROWS, NCOLS>::inverse () const
{
...
//////////////////////////////////////////////////////////////////////////////
template <typename value_t, unsigned NROWS, unsigned NCOLS>
matrix<value_t, NCOLS, NROWS>
matrix<value_t, NROWS, NCOLS>::transpose  () const
{
...
matrix<value_t, NROWS, NCOLS>::valid () const
{
  for ( std::size_t r = 0; r != NROWS; ++r )
  {
    for ( std::size_t c = 0; c != NCOLS; ++c )
    {
      if ( _data[r][c] == std::numeric_limits<value_type>::infinity() ||
           _data[r][c] == std::numeric_limits<value_type>::quiet_NaN() ||
           _data[r][c] == std::numeric_limits<value_type>::signaling_NaN() )
      {
        return false;
      }
    }
...
value_t
compute_determinant ( matrix<value_t, N, N> const& M )
{
...
template <typename value_t>
value_t
compute_determinant ( matrix<value_t, 3, 3> const& M )
{
  return value_t( M[0][0] * M[1][1] * M[2][2] +
                  M[0][1] * M[1][2] * M[2][0] +
                  M[0][2] * M[1][0] * M[2][1] -
                  M[0][2] * M[1][1] * M[2][0] -
                  M[0][1] * M[1][0] * M[2][2] -
...
matrix<value_t, 3, 3> compute_inverse(matrix<value_t, 3, 3> const& rhs)
{
  ...
  if (det == value_t(0)) {
    //throw std::runtime_error("Matrix could not be inverted");
    std::cout << "matrix<T,N,M>::compute_inverse() : Warning, matrix could not be inverted.\n";
  }
...
//////////////////////////////////////////////////////////////////////////////
template <typename value_t, unsigned N>
std::set<value_t> compute_eigenvalues ( matrix<value_t, N, N> const& M, value_t const& min, value_t const& max, value_t const& tolerance )
{
...
//////////////////////////////////////////////////////////////////////////////
template <typename value_t, unsigned N>
std::vector<point<value_t, N> > compute_eigenvectors ( matrix<value_t, N, N> const& m, value_t const& min, value_t const& max, value_t const& eps )
{
  ...
  for(value_t const& eigenval : eigenvals)
  {
    ...
    for ( unsigned r = 0; r != N; ++r )
    {
      ...
      if ( pivot.second != N )
      {
        ...
        for ( unsigned row = r+1; row != N; ++row )
        {
          ...
          if ( C[row][pivot.first] != 0 ) {
            ...
            {
              C[row][col] = ( col <= pivot.first ) ? 0 : C[row][col] + factor * C[r][col];
            }
          }
        }
      }
      ...
      if ( pivot.second != N )
      {
        ...
        if ( nonfree_parameter.find(r) == nonfree_parameter.end() ) { // if not in nonfree_map -> add as free parameter
          free_parameter.insert(r);
        }
      }
    }
    ...
    if ( rank == N )
    {
      ...
      for(unsigned const& free : free_parameter)
      {
    ...
    for ( std::map<unsigned, unsigned>::const_reverse_iterator pivot = nonfree_parameter.rbegin(); pivot != nonfree_parameter.rend(); ++pivot)
    {
      value_t rhs (0);
      for ( unsigned c = 0; c != N; ++c )
      {
...
//////////////////////////////////////////////////////////////////////////////
template <typename value_t, unsigned NROWS, unsigned NCOLS>
matrix<value_t, NROWS-1, NCOLS-1>
generate_submatrix ( matrix<value_t, NROWS, NCOLS> const& m, unsigned row, unsigned col )
{
...
template <typename value_t>
matrix<value_t, 1, 1>
generate_submatrix ( matrix<value_t, 1, 1> const& m, unsigned /*row*/, unsigned /*col*/ )
{
  return m;
}
...
//////////////////////////////////////////////////////////////////////////////
template <typename value_t, unsigned NROWS, unsigned NCOLS>
bool operator==(matrix<value_t, NROWS, NCOLS> const& lhs, matrix<value_t, NROWS, NCOLS> const& rhs)
{
  for ( std::size_t r = 0; r != NROWS; ++r ) {
    for ( std::size_t c = 0; c != NCOLS; ++c ) {
...
point_t
operator*(matrix<value_t, NROWS, NCOLS> const& m, point_t const& p)
{
...
//////////////////////////////////////////////////////////////////////////////
template <typename value_t, unsigned NROWS_A, unsigned NCOLS_A, unsigned NROWS_B, unsigned NCOLS_B>
matrix<value_t, NROWS_A, NCOLS_B>
operator*(matrix<value_t, NROWS_A, NCOLS_A> const& lhs, matrix<value_t, NROWS_B, NCOLS_B> const& rhs)
{
...
//////////////////////////////////////////////////////////////////////////////
template <typename value_t, unsigned NROWS, unsigned NCOLS>
matrix<value_t, NROWS, NCOLS> operator*(matrix<value_t, NROWS, NCOLS> const& lhs, value_t const& rhs)
{
...
//////////////////////////////////////////////////////////////////////////////
template <typename value_t, unsigned NROWS, unsigned NCOLS>
matrix<value_t, NROWS, NCOLS> operator*(value_t const& lhs, matrix<value_t, NROWS, NCOLS> const& rhs)
{
...
//////////////////////////////////////////////////////////////////////////////
template <typename value_t, unsigned NROWS, unsigned NCOLS>
matrix<value_t, NROWS, NCOLS> operator-(matrix<value_t, NROWS, NCOLS> const&  lhs, matrix<value_t, NROWS, NCOLS> const& rhs)
{
  matrix<value_t, NROWS, NCOLS> tmp(lhs);
...
//////////////////////////////////////////////////////////////////////////////
template <typename value_t, unsigned NROWS, unsigned NCOLS>
std::ostream& operator<<(std::ostream& os, matrix<value_t, NROWS, NCOLS> const& m)
{
  m.print(os);
  return os;
}
...
#endif // GPUCAST_MATH_MATRIX_IMPL_HPP
