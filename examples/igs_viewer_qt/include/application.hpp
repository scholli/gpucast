...
*
********************************************************************************/
#ifndef VOLUME_VIEW_APPLICATION_HPP
#define VOLUME_VIEW_APPLICATION_HPP
...
public :
  ...
  void init_data()
  {
    ...
    tml::point<float, 6> p00(&d00[0], &d00[6]);
    tml::point<float, 6> p01(&d01[0], &d01[6]);
    tml::point<float, 6> p02(&d02[0], &d02[6]);

    tml::point<float, 6> p10(&d10[0], &d10[6]);
    tml::point<float, 6> p11(&d11[0], &d11[6]);
    tml::point<float, 6> p12(&d12[0], &d12[6]);

    tml::point<float, 6> p20(&d20[0], &d20[6]);
    tml::point<float, 6> p21(&d21[0], &d21[6]);
    tml::point<float, 6> p22(&d22[0], &d22[6]);

    tml::point<float, 6> p30(&d30[0], &d30[6]);
    tml::point<float, 6> p31(&d31[0], &d31[6]);
    tml::point<float, 6> p32(&d32[0], &d32[6]);
    ...
    tml::point<float, 6> c0(&dc0[0], &dc0[6]);
    tml::point<float, 6> c1(&dc1[0], &dc1[6]);
    tml::point<float, 6> c2(&dc2[0], &dc2[6]);
    tml::point<float, 6> c3(&dc3[0], &dc3[6]);
    tml::point<float, 6> c4(&dc4[0], &dc4[6]);
    ...
    for ( tml::beziervolume<tml::point<float, 6> >::const_point_iterator cp = volume.begin(); cp != volume.end(); ++cp) 
    {
      indices_points.push_back(attrib0.size());
  ...
  void draw()
  {
    ...
    _indexbuffer_samples.set_index_pointer(GL_INT, 0, 0);
    ...
    _indexbuffer_points.set_index_pointer(GL_INT, 0, 0);
